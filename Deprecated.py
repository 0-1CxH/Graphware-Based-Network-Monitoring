...
# ----------------------------- Older Version of Packet Object --------------------------------
# ----------------------------- Replaced by <dpktOctet> ---------------------------------------
class Packet(object):
    ...
    def __init__(self, timestamp, ip_datagram):
        ...
        if self.proto in [1]:  # ICMP
            i f(ip_datagram.data.type in [0 ,8 ,13 ,14 ,15 ,16 ,17 ,18 ,30]):
            ...
            if self.prot o= =6:
                self.__isTCP = True
                # (ACK,RST,SYN,FIN)
                self.xInfo = ((transport_segment.flag s> >4 ) &1, (transport_segment.flag s> >2 ) &1,
                              (transport_segment.flag s> >1 ) &1, (transport_segment.flag s> >0 ) &1 )
            else:
    ...
    def getSrcPort(self):
        if self.__isTransportLaye r= =False:
            return None
        else:
            return self.sport
    ...
    def debug_printPacket(self):
        print(self.sip, self.getSrcIP(), self.getSrcPort(), self.dip, self.getDstIP(), self.getDstPort(),
              self.proto, self.protoName, self.length, self.getXInfo() ,
              self.timestamp, self.getUTCTime())
# ------------------------------------------------------------------------------------------------------

# ------------- Slow Filter, Replaced by More Rigorous and Faster <dpktFilter> -------------------------
class PacketFilter(object):

    def __init__(self, packetList, ipFilter=None, portFilter=None, timeFilter=None, lengthFilter=None,
                 protoFilter=None):
...
# ------------------------------ Iteration Version of EFP/wEFP Converter -----------------------------
# ------------------------------ Replaced by Increment Version (Combined) ----------------------------------
...
class EFP_Edge(object):
    ...
    def __eq__(self, other):
        if (self.head in [other.head ,other.tail]) and \
                (self.tail in [other.head,other.tail]) and             self. linkTyp e= =other.linkType:
            return True
...
class EFPConverter(object):
    ...
    def makePyVisObject(self):
        self.PvObj = pyvis.network.Network(height="100%" ,width="100%" ,directed=True ,notebook=False
...
class wEFPConverter(object):
    ...
    def makeJSON(self):
        ...
        for v in self.VertexTable:
            self.jsondata['nodes'].append({'id': v.hostID})
        for e in self.EdgeTable:
            self.jsondata['links'].append(
                {'source': e.tail, 'target': e.head, 'value': e.extraInfo, 'type': e.linkType})
        return json.dumps(self.jsondata)
    ...
    def makeNetworkxObject(self):
        ...
        for v in self.VertexTable:
            self.NxObj.add_node(v.hostID)
        for e in self.EdgeTable:
            self.NxObj.add_edge(e.tail, e.head, type=e.linkType, count=e.extraInfo['count'],
                                throughput=e.extraInfo['throughput'], duration=e.extraInfo['duration'])
        return self.NxObj
    ...
    def makePyVisObject(self):
        self.PvObj = pyvis.network.Network(height="100%", width="100%", directed=False, notebook=False,
                                           bgcolor="#AAAAAA", font_color="white")
        ...
        for e in self.EdgeTable:
            ...
            self.PvObj.add_edge(e.tail, e.head, title=e.extraInfo['count'], value=self.calculateWidth(e),
                                color=self.colorD[e.linkType])
        return self.PvObj
# ------------------------------------------------------------------------------------------------------
